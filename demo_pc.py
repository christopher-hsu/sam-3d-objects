...
def load_pc_or_depth2pc_and_accumulate(dataset_dir, scene_graphs, buffer):
    init_odom = None
    for steps in scene_graphs.keys():
        # for lidar data
        pc_path = os.path.join(dataset_dir, steps, "points.ply")
        if os.path.exists(pc_path):
            print(f"Loading point cloud from {pc_path}")
            pcd = o3d.io.read_point_cloud(pc_path)

            if init_odom is None:
                odom_path = os.path.join(dataset_dir, steps, "odom.json")
                with open(odom_path, "r") as f:
                    odom = json.load(f)
                init_odom = np.array([odom["t"][0], odom["t"][1], 0.0])
            
            pcd_points = np.array(pcd.points) - init_odom
            buffer.add_points(np.array(pcd_points))

        else: #for realsense data
            depth_path = os.path.join(dataset_dir, steps, "depth.png")
            cam_path = os.path.join(dataset_dir, steps, "camera_info.json")
            odom_path = os.path.join(dataset_dir, steps, "odom.json")
            if os.path.exists(depth_path) and os.path.exists(cam_path) and os.path.exists(odom_path):
                ...
                Z[Z <= 0] = np.nan
                Z[Z > 5.0] = np.nan  # optional: filter out points beyond 10 meters
                X = (uu - cx) * Z / fx
                Y = (vv - cy) * Z / fy
                ...
                buffer.add_points(np.array(pcd.points))
                # yield pcd
            else:
                print(f"No point cloud or depth image found for step {steps}")
                continue
...
def accum_pc(dataset_dir):
    ...
    # load pc
    buffer = load_pc_or_depth2pc_and_accumulate(dataset_dir, scene_graphs, buffer)

    # save accumulated pc
    out_pc_path = os.path.join(dataset_dir, "accumulated_points.ply")
    o3d.io.write_point_cloud(out_pc_path, buffer.pcd)
    print(f"Saved accumulated point cloud to {out_pc_path}")

    # saved meshes and pc together
    all_meshes = meshes.copy()
    pc_mesh = trimesh.PointCloud(buffer.get_points())
    all_meshes.append(pc_mesh)
    # combined = trimesh.util.concatenate(all_meshes)
    scene = trimesh.Scene(all_meshes)
    out_combined_path = os.path.join(dataset_dir, "combined_mesh_and_pc.glb")
    # scene.export(out_combined_path)
